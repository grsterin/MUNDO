...
def main(args):
    def log(strng):
        if args.verbose:
            print(strng)
    mash_map = {}
    with open(f"{args.input_folder}/{args.n2vec_emb}.json", "r") as jf:
        n2_map = json.load(jf)
    
    E        = np.abs(np.load(f"{args.input_folder}/{args.n2vec_emb}.n2vec.npy"))
    
    r_n2_map = {val:key for key, val in n2_map.items()}
    
    nlist = [int(r_n2_map[i]) for i in range(len(r_n2_map))]
    ...
    prot_go = get_prot_go_dict(go_prots_dict, n2_map)
