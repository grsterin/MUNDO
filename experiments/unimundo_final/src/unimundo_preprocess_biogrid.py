...
def main(args):
    ...
    with open(f"{args.working_folder}/{args.src_simple_organism_name}-biogrid-to-entrez.json") as file:
        json.dump(src_biordid_to_entrez_dict, file)
    with open(f"{args.working_folder}/{args.tgt_simple_organism_name}-biogrid-to-entrez.json") as file:
